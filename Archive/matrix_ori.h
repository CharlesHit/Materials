...
vec mix(const vec &a, const vec& b, const float &mixValue)
...
float clamp(const double &lo, const double &hi, const double &v)
...
void fresnel(const vec &I, const vec &N, const double &ior, double &kr)
{
	float cosi = clamp(-1, 1, (I * N));
	float etai = 1, etat = ior;
	...
	if (sint >= 1) {
		...
		double cost = sqrtf(std::max(0.f, 1 - sint * sint));
		cosi = fabsf(cosi);
...
public:
	...
	inline vec2 ( double x, double y):l(2), c(1)
	{
		m.resize(3, vector<double>(2, 0));
		m[1][1] = x;
		m[2][1] = y;
	}
	inline vec2 ( const vec &v )
	{
		m.resize(3, vector<double>(2, 0));
		m[1][1] = v.m[1][1];
		m[2][1] = v.m[2][1];
	...
	//vec + vec, vec - vec
	inline vec2 operator+(const vec &b )
	{
		return vec2 { m[1][1] + b.m[1][1], m[2][1] + b.m[2][1] };
	}
	inline vec2& operator+=(const vec &b )
	{
		m[1][1] += b.m[1][1];
	...
	inline vec2 operator-( vec &b )
	{
		return vec2 { m[1][1] - b.m[1][1], m[2][1] - b.m[2][1] };
	}
	friend vec2 operator-( const vec2 &a, const vec2 &b )
	{
		return vec2 { a.m[1][1] - b.m[1][1], a.m[2][1] - b.m[2][1]};
	}
//vec * num, num* vec, vec*vec (dot product)
	inline vec2 operator*(double s )
	{
		return vec2 { m[1][1] * s, m[2][1] * s };
	}
	//notice here
	...
	friend double operator*( const vec2 &v, const vec2 &w  )
	{
...
//vec / num
	...
	{
		return vec2 { s / v.m[1][1], s / v.m[2][1] };
	}
// vec = -vec
	inline vec2 operator-()
	{
		return vec2 { -m[1][1], -m[2][1] };
	}
